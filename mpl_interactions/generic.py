...
def heatmap_slicer(X,Y,heatmaps,slices='horizontal',heatmap_names = None,max_cols=None,cmap=None,figsize=(18,9),linecolor='k',labels=('X','Y'),interaction_type='move'):
    ...
    for i,ax in enumerate(axes[:-num_line_axes]):
        ax.pcolormesh(X,Y,heatmaps[i],cmap=cmap,shading=shading)
